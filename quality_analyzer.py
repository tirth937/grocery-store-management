...
def get_quality_info(class_name: str, img: np.ndarray = None, box: list = None, is_custom_model: bool = False):
    ...
    ----------
    class_name : str
        The class label from YOLO (e.g. 'apple' or 'fresh_apple')
    img : np.ndarray, optional
        Full image (BGR). Needed for heuristic mode.
    box : list, optional
        [x1, y1, x2, y2] bounding box. Needed for heuristic mode.
    is_custom_model : bool
        If True, parse compound labels. If False, use heuristic.

    Returns
    -------
    dict with keys: food_name, quality_label, recommendation, shelf_life_days, color_code
    """
    ...
    if quality_key is None and img is not None and box is not None:
        ...
        quality_key = analyze_quality_heuristic(crop, food_name)
    ...
    # Map quality_key to shelf life bucket
